...
function PortfolioTiles() {
    return (
        <>
            {portfolio.map(item => (
                <div key={item.id} class="col-10 col-sm-8 col-md-5 col-lg-3 pr-0 pl-0 mt-5 tile" style={{ position: "relative" }}>
                    <img src={item.image} alt={item.imageAlt} style={{ width: "100%" }} />
                    <div class="box">
                        <a href={item.site} target="blank">
                            <h5 style={{ textAlign: "center;" }}>{item.name}</h5>
                        </a>
                        <img src={item.icon} alt={item.iconAlt} class="tileIcon text-center" />
                        <p>{item.type}</p>
